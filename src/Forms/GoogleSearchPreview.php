...
class GoogleSearchPreview extends LiteralField
{
    ...
    public function __construct($name, $title, $page, simple_html_dom $domParser)
    {
		...
		$titlenode = $domParser->find('title', 0);
        $renderedTitle = $titlenode ? $titlenode->innertext() : '';
		
		// theres a problem here that the function MetaTitle() doesn't fire, possibly due to order extension is loaded - not sure.
		if(empty($renderedTitle)){
			$renderedTitle = $page->defaultMetaTitle();
		}
